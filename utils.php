...
function fileToNote($fileName) {
	...
	foreach ( $root->getElementsByTagName("section") as $key => $value) {
		$currSection = new stdClass();
		$currSection->id = $value->getAttribute("id");
		$currSection->title = $value->getAttribute("title");
		
		$notes = array();
		foreach ( $value->getElementsByTagName("note") as $noteKey => $noteValue) {
			$currNote = new stdClass();
			$currNote->content = $noteValue->nodeValue;
			$notes[$noteKey] = $currNote;
			/*echo $currNote->content;*/
		}
		$currSection->notes = $notes;
