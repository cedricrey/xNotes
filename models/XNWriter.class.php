<?php
   class XNWriter{
		...
		static public function NotebookToXML(Notebook $notebook, DOMDocument $dom){
			...
			/*
			if($notebook->getIdSequence() != "")
			{
				$idSequenceAttribute = $dom->createAttribute('idSequence');
				$idSequenceAttribute->value = $notebook->getIdSequence();
				$xmlNoteBook->appendChild($idSequenceAttribute);
			}
			*/
			return $xmlNoteBook;
		}
		...
		static public function ElementToXML(Element $element, DOMDocument $dom){
			...
			if($element->getId() != "")
			{
				...
				$idAttribute->value = $element->getId();
				$xmlElement->appendChild($idAttribute);	
			}
			if($element->getTitle() != "")
			{
